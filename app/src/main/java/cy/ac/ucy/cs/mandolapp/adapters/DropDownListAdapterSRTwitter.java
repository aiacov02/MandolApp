...
public class DropDownListAdapterSRTwitter extends BaseAdapter {
    ...
    public void setSelected(String selected) {
        DropDownListAdapterSRTwitter.selected = selected;
    }

    public DropDownListAdapterSRTwitter(Context context, ArrayList<String> items,
                                        Button btn) {
        ...
        this.selectedCount = SavedReportsTwitter.selectedCategoriesNumber;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(SavedReportsTwitter.checkSelected[position])
    ...
    public void setText(int position1){
        if (!SavedReportsTwitter.checkSelected[position1]) {
            SavedReportsTwitter.checkSelected[position1] = true;
            selectedCount++;
        } else {
            SavedReportsTwitter.checkSelected[position1] = false;
            selectedCount--;
        }
        SavedReportsTwitter.selectedCategoriesNumber = selectedCount;

        if (selectedCount == 0) {
            mSelectedItems.setText("Choose Category");
        } else if (selectedCount == 1) {
            for (int i = 0; i < SavedReportsTwitter.checkSelected.length; i++) {
                if (SavedReportsTwitter.checkSelected[i] == true) {
