...
public class DropDownListAdapterPhoto extends BaseAdapter {
    ...
    public void setSelected(String selected) {
        DropDownListAdapterPhoto.selected = selected;
    }

    public DropDownListAdapterPhoto(Context context, ArrayList<String> items,
                                    Button btn) {
...
//        selectedCount=0;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(ReportPhoto.checkSelected[position])
    ...
    private void setText(int position1){
        ...
        if (!ReportPhoto.checkSelected[position1]) {
            ReportPhoto.checkSelected[position1] = true;
            selectedCount++;
        } else {
            ReportPhoto.checkSelected[position1] = false;
            selectedCount--;
        }
        ...
        if (selectedCount == 0) {
            ...
            System.out.println(selectedCount);
            for (int i = 0; i < ReportPhoto.checkSelected.length; i++) {
                if (ReportPhoto.checkSelected[i] == true) {
