...
public class SearchFieldAdapter extends BaseAdapter {
    ...
    public void setSelected(String selected) {
        SearchFieldAdapter.selected = selected;
    }

    public SearchFieldAdapter(Context context, ArrayList<String> items,
                              Button btn) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(ReportHistory.checkSelectedField[position])
    ...
    private void setText(int position1){
        if (!ReportHistory.checkSelectedField[position1]) {

            ReportHistory.checkSelectedField[position1] = true;
            System.out.println("position " + position1);
            if(position1==1){
                ReportHistory.checkSelectedField[(position1-1)] = false;
            }
            else{
                ReportHistory.checkSelectedField[(position1+1)] = false;
            }
        } else {
            ReportHistory.checkSelectedField[position1] = false;
        }
        EditText txtSearch = (EditText)((Activity)context).findViewById(R.id.txtSearch);
        if(ReportHistory.checkSelectedField[0]){
