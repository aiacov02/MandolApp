...
public class DropDownListAdapterFacebook extends BaseAdapter {
    ...
    public void setSelected(String selected) {
        DropDownListAdapterFacebook.selected = selected;
    }

    public DropDownListAdapterFacebook(Context context, ArrayList<String> items,
                               Button btn) {
...
//        selectedCount=0;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(ReportFacebook.checkSelected[position])
    ...
    private void setText(int position1){
        if (!ReportFacebook.checkSelected[position1]) {
            ReportFacebook.checkSelected[position1] = true;
            selectedCount++;
        } else {
            ReportFacebook.checkSelected[position1] = false;
            selectedCount--;
        }
        ...
        if (selectedCount == 0) {
            ...
            for (int i = 0; i < ReportFacebook.checkSelected.length; i++) {
                if (ReportFacebook.checkSelected[i] == true) {
